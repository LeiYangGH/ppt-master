...
4. **⛔ 批量缩略图审阅 + 重命名（阻断性）**：
   ...
   **第一步：生成缩略图墙**（每 20 张拼成一张 montage，便于一次视觉读图判定批量图片）：
   ```bash
   python3 ${SKILL_DIR}/scripts/image_montage.py --project-dir <topic_name>
   ```
   ...
   **第三步（可选）：精查少数可疑图**。仅针对缩略图中看不清细节的少数图运行：
   ```bash
   python3 ${SKILL_DIR}/scripts/analyze_images.py projects/<topic_name>/images/<suspect_file>.jpg
   ```
