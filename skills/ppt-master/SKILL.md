...
> 8. **每页重读 SPEC_LOCK** —— 在生成每页 SVG 之前，Executor **必须** `read_file <project_path>/spec_lock.md`。所有颜色 / 字体 / 图标 / 图片**必须**来自此文件。Executor 还**必须**查找当前页的 `page_rhythm` 标签，并应用匹配的布局纪律（`structural` / `analytical` / `focal`——见 executor-base.md §2.1）。
...
> - **直接读写文件**：当前运行于 opencode CLI 环境，具备原生文件读写能力。**必须**使用 opencode 提供的文件读写工具直接操作文件，**禁止**通过新建 Python 脚本等迂回方式间接读写文件。调用文件读写工具时须注意传参规范，确保路径正确、编码无误。
