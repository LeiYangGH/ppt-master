...
- 边距：40-80px
- 常见布局：多列、左右分栏、网格
- 卡片尺寸：单行 530-600px，双行 265-295px
...
本规范用于图片与**正文并排**放置、共同组成内容容器的页面。

**核心原则**：根据图片原始宽高比计算容器布局，保证图片完整显示——不留多余空白，不裁切。

### 布局决策流程

```
1. 先确定叙事意图（hero / atmosphere / side-by-side / accent）
2. 若意图 = side-by-side，则继续。否则按叙事需要自由构图。
...
画布参数：W=1160，H=600，边距 60px，标题高度 60px，内容起始 Y=80px
...
> 多图页面中，所有图片都应使用 `preserveAspectRatio="xMidYMid meet"`。
...
| 方法 | 优点 | 缺点 | 适用阶段 |
|--------|------|------|-------------|
| **外部引用** | 文件小、迭代快、易替换 | 预览时需启 HTTP 服务 | `svg_output/` 开发阶段 |
