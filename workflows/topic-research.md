...
| 2 | **自动下载的图片必须先批量缩略图审阅、确定用途、再重命名** | 搜索所得的原始文件名多为难以辨认的哈希字符串（如 `5f3a8c1b.jpg`），**禁止直接以此类名称在设计规范或 SVG 中引用**；你必须先运行 `image_montage.py` 将 `images/` 目录下所有图拼成 `montage_NN_of_MM.jpg` 缩略图墙（每张 20 格、格下标文件名），通过一次视觉读图批量判定（只对看不清 / 需细节确认的少数图才动用 `analyze_images.py` 精查），然后：<br>  • **适合使用** → 立即重命名为与图片内容一致的有意义英文名（如 `joe_hisaishi_portrait_2019.jpg`），避免后续因名称模糊出现**图片混用**；<br>  • **不合适** → 直接删除文件；<br>  • **执行完上述筛选后仍没有合适图片** → 换关键字继续搜索，直到满足设计需求为止。 |
...
```bash
# 全局概览搜索
python3 scripts/web_search.py "<主题> 概述" -n 8 --json

# 定向补充搜索
python3 scripts/web_search.py "<子主题> 数据 统计" -n 5 --json
```
...
**主工具** — `web_search.py`（搜索后**自动并发下载**图片到 `workspace/images/` 目录，5 秒/张超时，你无需再单独 `curl`）：

```bash
# 先告知脚本当前工作区目录（一次即可，后续所有搜索都会命中）
# - 方式 A：环境变量（推荐）
$env:PPT_PROJECT_DIR = "workspace"   # PowerShell
# export PPT_PROJECT_DIR=workspace   # bash
# - 方式 B：每次显式传参 --project-dir workspace
# - 方式 C：不设置时，脚本会自动选择 workspace/ 目录

# 搜索：返回 JSON 里每条结果含 images 列表，同时后台并发下载图片
python3 scripts/web_search.py "<主题> 相片" -n 8 --json
...
python3 scripts/web_search.py "<主题>" --no-auto-download

# 手动调参：自定义超时与上限
python3 scripts/web_search.py "<主题>" --auto-download-timeout 5 --auto-download-limit 30

# 下载成功/失败可回写给脚本用于域名排名（一般由自动下载自动调用，无需手动）
python3 scripts/web_search.py --record-download <domain> success|fail
python3 scripts/web_search.py --domain-stats
```

> 📌 **下载的图片文件名保留原始 URL 的 basename**，可能是随机码（如 `5f3a8c1b.jpg`）或就是 URL hash 回退名（如 `img_b0efe05aeb3b.jpg`）。后续你需要对 `workspace/images/` 下的图片**批量审阅**：先运行 `image_montage.py` 拼成缩略图墙，凭视觉批量判定采纳 / 删除 / 重命名；对缩略图中看不清细节的少数图再用 `analyze_images.py` 精查。
...
1. **确认工作区目录已识别**。搜索前通过 `PPT_PROJECT_DIR` 环境变量或 `--project-dir` 参数指定当前工作区（`workspace`），否则脚本会回退到 `workspace/` 目录。
2. **运行若干次 `web_search.py` 搜索（关键字必须中文，见顶部约束 #1）**。每次搜索结束，脚本会并发下载（8 线程 × 5s 超时，单次至多 30 张）到 `workspace/images/`，同一 URL 重复搜索不会重复下载。
3. **查看返回 JSON 的 `downloaded_images` 字段**，快速掌握哪些图片已落地、哪些失败（域名 403、超时、非图片 Content-Type 等）。
4. **⛔ 批量缩略图审阅 + 重命名（阻断性）**：

   **第一步：生成缩略图墙**（每 20 张拼成一张 montage，便于一次视觉读图判定批量图片）：
   ```bash
   python3 scripts/image_montage.py --project-dir workspace
   ```
   输出落在 `workspace/images/_montage/`：
   - `montage_01_of_MM.jpg`, `montage_02_of_MM.jpg` ... （每张 4×5 格，格下带文件名标签与左上角序号徽标）
   - `montage_manifest.json`（记录每张 montage 对应的源文件名清单，供你根据序号回查具体文件名）

   **第二步：逐张视觉读图**。依序读入每张 montage，凭缩略图 + 文件名标签批量给出保留 / 删除 / 待细查的初步判断。

   **第三步（可选）：精查少数可疑图**。仅针对缩略图中看不清细节的少数图运行：
   ```bash
   python3 scripts/analyze_images.py workspace/images/<suspect_file>.jpg
   ```

   **第四步：执行三选一**（不得留着原文件名进入下一步）：
   - **采纳** → 立即按 3.3.3 重命名为与图片内容一致的有意义名称（防止后续因随机哈希名导致**图片混用**）；
   - **弃用** → 直接删除文件，绝不留底；
   - **无任何图片合适** → 换一组中文关键字重新搜索，重复本步直至满足设计需求。

   > 完成本步前禁止进入步骤 4 或后续设计 / SVG 生成；未重命名的文件名（如 `5f3a8c1b.jpg`、`img_b0efe05aeb3b.jpg`）不得出现在 `design_spec.md` 或 SVG 引用中。
   > 审阅完毕后可删除 `_montage/` 整个目录（纯中间产物，不需进入最终交付）。
...
- **名称必须与图片实际内容一致**：凭缩略图墙的视觉内容（必要时补充 `analyze_images.py` 的精查结果）归纳主体（人物 / 场景 / 对象 / 事件）与上下文（年份 / 地点 / 主题），组合为文件名；
