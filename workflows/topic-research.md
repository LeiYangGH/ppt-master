...
> 📌 **下载的图片文件名保留原始 URL 的 basename**，可能是随机码（如 `5f3a8c1b.jpg`）或就是 URL hash 回退名（如 `img_b0efe05aeb3b.jpg`）。后续你需要对 `projects/<topic_name>/images/` 下的图片**批量审阅**：先运行 `image_montage.py` 拼成缩略图墙，凭视觉批量判定采纳 / 删除 / 重命名；对缩略图中看不清细节的少数图再用 `analyze_images.py` 精查。
...
4. **⛔ 批量缩略图审阅 + 重命名（阻断性）**：
   ...
   **第一步：生成缩略图墙**（每 20 张拼成一张 montage，便于一次视觉读图判定批量图片）：
   ```bash
   python3 scripts/image_montage.py --project-dir <topic_name>
   ```
   输出落在 `projects/<topic_name>/images/_montage/`：
   ...
   **第三步（可选）：精查少数可疑图**。仅针对缩略图中看不清细节的少数图运行：
   ```bash
   python3 scripts/analyze_images.py projects/<topic_name>/images/<suspect_file>.jpg
   ```
