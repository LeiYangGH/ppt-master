...
> 策略师：viewBox 和格式固定为 PPT 16:9，无需修改。
...
>
> **词汇表**（仅以下三个值）：
> - `structural` — 结构页（封面 / 章节开篇 / 目录 / 结尾）。严格按模板执行。
> - `analytical` — 信息密集型页面（数据、KPI、对比、多要点列表）。允许卡片网格、多列布局、表格、图表。
> - `focal` — 低密度页面（单一概念、hero 引言、大图 + 说明、章节过渡）。避免**多卡片网格布局**（多个并行圆角容器作为主要结构）；通过裸文本、分隔线、留白或全出血图片组织。单一圆角元素（hero 图片角、标注、标签、一个强调块）可以。比例遵循信息权重——不是预设比例菜单。
>
> **节奏跟随叙事**：`focal` 页面出现在叙事真正需要停顿的地方——章节过渡、值得独立强调的单一论点、密集序列后的刻意停顿。数据简报或咨询分析可能几乎全为 `analytical`——**不要发明填充页**来凑节奏。验证：每个 `focal` 页必须能回答"这一页在说什么独立的内容？"。
>
> **缺失或空节** → 执行器对每页回退到 `analytical`（遗留的前节奏行为）。仅对遗留幻灯片删除本节；新幻灯片必须填写。
