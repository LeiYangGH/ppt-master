...
**缺少 `page_rhythm` 段** → 输出一次 `warning: spec_lock.md missing page_rhythm — defaulting all pages to analytical`，并将所有页面回退为 `analytical`。
...
如果 `spec_lock.md` 缺失，请参考 `strategist.md` §g，**不要自己编造字体栈**。
