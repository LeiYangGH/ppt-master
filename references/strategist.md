...
| 意图 | 形式 | 使用时机 |
|--------|------|-------------|
| **Hero / full-bleed** | 图片占满画布或主导区，标题通过渐变或透明覆盖浮在上方 | 封面、章节过渡页、`focal` 页面——图片本身就是信息 |
...
> **原则——比例应服从信息重量，而不是预设比例。** 可以组合模式、在 `focal` 页面中打破网格，或者提出新的模式。若所有页面都默认对称网格，会很容易出现"AI 生成感"。
...
| 全幅图 + 浮动文字 | `focal` / 特写页 | 图片占满 1280x720，文字浮在透明覆盖层上 |
